...
public class StageButtonManager : MonoBehaviour
{
    ...
    public class StageButtonData
    {
        ...
        public GameObject[] starIcons;               // 별 오브젝트 (최대 3개)
        public GameObject lockOverlay;               // 잠금 상태용 덮개 이미지 (검은 이미지 등)
    }
    ...
    private void Start()
    {
        foreach (var data in stageButtons)
        {
            ...
            Debug.Log($"⭐ {data.stageKey} → {PlayerPrefs.GetInt(data.stageKey)}개");
